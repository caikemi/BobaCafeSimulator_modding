...
新增 `FDrinkData` 时需要特别注意以下字段：

| 字段 | 类型 | 说明 |
|---|---|---|
| `DrinkType` | `EDrinkType` | `FDrinkData` 默认是 `MilkTea`；应根据饮品显式设置，可选值见下表。 |
| `Season` | `TArray<EGBSeason>` | 默认是空数组；空数组可能导致配方不在任何季节出现，应按需求添加春、夏、秋、冬。 |
`EDrinkType` 的全部可选值：
...
奶茶店当前使用的 Steam App ID 是 **`3683770`**。SteamCMD 的官方说明可参考 [Steam Workshop Implementation Guide](https://partner.steamgames.com/doc/features/workshop/implementation)。

上传前确认 Mod 文件夹中直接包含 `main.lua`、`preview.png` 和运行时资源，不要在 `contentfolder` 下额外套一层同名目录。

### 1. 创建 VDF

新建 `LocalizedPumpkinDrink.vdf`：
...
"workshopitem"
{
    "appid"            "3683770"
    "publishedfileid"  "0"
    "contentfolder"    "D:\\BobaMods\\LocalizedPumpkinDrink"
    "previewfile"      "D:\\BobaMods\\LocalizedPumpkinDrink\\preview.png"
    "visibility"       "0"
    "title"            "本地化南瓜汁示例"
...
- 首次创建时将 `publishedfileid` 设为 `0`。
- 上传成功后 SteamCMD 会把生成的创意工坊物品 ID 写回 VDF。
- 后续更新继续使用同一份 VDF 和已经生成的 `publishedfileid`。
- `visibility = 0` 表示公开；首次测试时可以根据需要改为非公开可见性。

### 2. 使用 SteamCMD 上传

```powershell
steamcmd.exe +login YourSteamAccount +workshop_build_item "D:\BobaMods\LocalizedPumpkinDrink.vdf" +quit
```

SteamCMD 会提示输入密码和 Steam Guard 验证码。不要把账号密码明文写进 `.bat`、VDF 或提交到仓库。

### 3. 查看创意工坊条目与本地缓存

创意工坊文件路径：

`[Steam 安装目录]\steamapps\workshop\content\3683770`

该目录下每个数字文件夹对应一个创意工坊物品 ID。将数字追加到以下地址，即可打开对应条目：
